...
def train_chemberta_ensemble(df_train, df_test, y_true, target_name, seed, n_splits=5, epochs=6, batch_size=16, lr=2e-5):
    torch.manual_seed(seed)
    np.random.seed(seed)
...
submission.to_csv('outputs/submission_v11.csv', index=False)
print('\noutputs/submission_v11.csv saved.')
...
plt.suptitle(f'v11 OOF Predicted vs Actual  |  Mean R² = {(r2_tg+r2_egc)/2:.4f}', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('outputs/oof_scatter_v11.png', dpi=120, bbox_inches='tight')
print('Plot saved as outputs/oof_scatter_v11.png.')
