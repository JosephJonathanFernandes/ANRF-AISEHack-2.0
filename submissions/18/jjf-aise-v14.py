...
for seed in TRANSFORMER_SEEDS:
    oof_p, test_p, oof_e, test_e = train_transformer_ensemble(train_tg, test_tg, y_tg, "Tg", seed, model_name)
...
oof_e_tg  = np.mean(polybert_oof_tg_embs_all, axis=0)
test_e_tg = np.mean(polybert_test_tg_embs_all, axis=0)

polybert_oof_egc_all, polybert_test_egc_all = [], []
polybert_oof_egc_embs_all, polybert_test_egc_embs_all = [], []
for seed in TRANSFORMER_SEEDS:
    oof_p, test_p, oof_e, test_e = train_transformer_ensemble(train_egc, test_egc, y_egc, "Egc", seed, model_name)
...
oof_e_egc  = np.mean(polybert_oof_egc_embs_all, axis=0)
test_e_egc = np.mean(polybert_test_egc_embs_all, axis=0)
...
# -----------------------------------------------------------------------------
# 5. TABULAR MODELS (LGBM + XGB) with PolyBERT Embeddings appended
# -----------------------------------------------------------------------------
...
print('  v14 ENSEMBLE')
print('=' * 60)
print(f'Optimal Tg  weights — LGBM: {w_tg[0]:.3f}  XGB: {w_tg[1]:.3f}  Large GNN: {w_tg[2]:.3f}  PolyBERT-FT: {w_tg[3]:.3f}')
print(f'Optimal Egc weights — LGBM: {w_egc[0]:.3f}  XGB: {w_egc[1]:.3f}  Large GNN: {w_egc[2]:.3f}  PolyBERT-FT: {w_egc[3]:.3f}')
...
submission.to_csv('outputs/submission_v14.csv', index=False)
print('\noutputs/submission_v14.csv saved.')
...
plt.suptitle(f'v14 OOF Predicted vs Actual  |  Mean R² = {(r2_tg+r2_egc)/2:.4f}', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('outputs/oof_scatter_v14.png', dpi=120, bbox_inches='tight')
print('Plot saved as outputs/oof_scatter_v14.png.')
