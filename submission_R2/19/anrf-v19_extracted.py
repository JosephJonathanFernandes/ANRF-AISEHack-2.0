...
NAME = "sibling"
N_FOLDS = 10
N_AUG = 8          # masked replicas of the training set (robustness augmentation)
...
def build_stage_a_compact():
    ...
    for t in TARGETS:
        ...
        P_oof = np.column_stack([oof_p1, oof_p2])
        import scipy.optimize as _sopt
        w_sa, _ = _sopt.nnls(P_oof * sd + mu, y)
        w_sa = w_sa / w_sa.sum() if w_sa.sum() > 1e-9 else np.array([0.5, 0.5])
        out[t] = (w_sa[0] * f1 + w_sa[1] * f2) * sd + mu
        oof_blended = (oof_p1 * w_sa[0] + oof_p2 * w_sa[1]) * sd + mu
        out.loc[s.smiles.values, t] = oof_blended
        print(f"  stageA {t:>4}  n={len(y):<5} oof R2={r2(y, oof_blended):.4f}  w_lgb={w_sa[0]:.2f} w_kr={w_sa[1]:.2f}")
...
for t in TARGETS:
    ...
    # Pseudo labels from Stage A for the test set
    y_pseudo = stageA.loc[test.smiles.values, t].to_numpy(float)
    ...
    for k in range(nf):
        ...
        # We add the test set with Stage A's predictions to the training mix
        Xs.append(make_features(test.smiles.values, t, tbl, rng=rng, mask_p=0.0))
        ys.append(y_pseudo)
    ...
    # Semi-Supervised Learning for full fit
    Xs.append(make_features(test.smiles.values, t, full_table, rng=rng, mask_p=0.0))
    ys.append(y_pseudo)
