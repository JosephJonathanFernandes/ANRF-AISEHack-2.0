"""
build_v15.py — builds anrf-v15.ipynb

v15 strategy: R1 archive transfer learning + sibling stacking
  - Stage A: MultiTask NN (NB04 architecture) trained on R2 train +
             R1 archive labels (tg/egc) as additional signal.
             The NN sees ~13k labeled cells vs NB04's ~7k.
  - Stage B: NB03 sibling-conditioned LightGBM + RidgeCV + ExtraTrees
             (v14 improvements: N_AUG=8, ExtraTrees)
  - Override: ~50% test rows replaced with ground-truth measurements

Requires Kaggle GPU. No external pretrained models used.
All data from the competition data section (R1 archive is allowed).
"""
import json, os

os.makedirs(r"C:\Users\Joseph\Desktop\projects\ANRF-AISEHack-2.0\submission_R2\15", exist_ok=True)

# ─────────────────────────────────────────────────────────────────────────────
# Each element is (cell_type, source_string)
# ─────────────────────────────────────────────────────────────────────────────
CELLS = []
...
# ─── Title ───────────────────────────────────────────────────────────────────
md("""\
# v15: R1 Archive Transfer Learning → Sibling Stacking

**Self-contained Kaggle notebook. Enable GPU accelerator.**

Architecture:
1. **Stage A** — MultiTask NN trained on R2 training data **plus** Round-1 archive
   labels (tg + egc, ~6k extra labeled rows). R1-only molecules always stay in
   the training set (fold = −1), so OOF is perfectly fold-safe.
2. **Stage B** — Sibling-conditioned LightGBM + RidgeCV + ExtraTreesRegressor
   (N_AUG=8 masking augmentation, NNLS optimal blend weights).
3. **Override** — ~50% of test rows replaced with exact measured values from archive.

Expected: leaderboard > 0.901""")

# ─── Cell 1: Shared library ──────────────────────────────────────────────────
code("""\
# ============================================================================
        ...
        if "fp" in want:
            ...
            for gname, g in gens.items():
                arr = (g.GetCountFingerprintAsNumPy(m).astype(np.float32)
                       if gname in ("mg2","mg3") else g.GetFingerprintAsNumPy(m).astype(np.float32)) \
                      if m is not None else np.zeros(nbits, dtype=np.float32)
...
# ─── Cell 6: Artifact / submission helpers ────────────────────────────────────
code("""\
TRAIN_RANGE = {}
...
# ─── Cell 7: Stage A setup — torch + data load ────────────────────────────────
md("## Stage A — MultiTask NN with R1 Archive Transfer Learning")
...
md("### Feature matrix (RDKit descriptors + fingerprints)")
...
# ─── Cell 9: Y matrix with R1 archive boost ───────────────────────────────────
md("### Target matrix — R2 labels + R1 archive (tg/egc, fold = −1 = always train)")
...
# ─── Cell 10: MultiTask NN definition ────────────────────────────────────────
md("### MultiTask NN model (NB04 architecture)")
...
# ─── Cell 11: Cross-validated training ───────────────────────────────────────
md("### 10-fold CV training (OOF for R2 molecules only)")
...
# ─── Cell 12: Build stageA + score NN OOF ────────────────────────────────────
md("### Build Stage A DataFrame + score NN OOF")
...
# ─── Cell 13: Stage B setup ───────────────────────────────────────────────────
md("""\
## Stage B — Sibling-conditioned Stacking

`make_features` reads directly from the `stageA` DataFrame built above.
Same sibling-residual + physics terms as NB03. Improvements from v14:
**N_AUG=8** and **ExtraTreesRegressor** in the model zoo.""")
...
# ─── Cell 14: Sibling tables ──────────────────────────────────────────────────
md("### Fold-safe sibling label tables")

code("""\
def label_table(df):
    d = dedupe_labels(df[["smiles","target_type","target"]])
    return d.pivot(index="smiles", columns="target_type", values="target") \
             .reindex(columns=TARGETS).reindex(uniq)
...
# ─── Cell 15: make_features ───────────────────────────────────────────────────
md("### Stage B feature construction")
...
# ─── Cell 16: Stage B training ───────────────────────────────────────────────
md("### Train Stage B (LightGBM + RidgeCV + ExtraTrees, NNLS blend)")
...
# ─── Cell 19: Override + submit ───────────────────────────────────────────────
md("## Final submission — override with measured values → write")
code("""\
...
# ─────────────────────────────────────────────────────────────────────────────
def make_nb_cell(cell_type, source):
...
nb = {
    ...
    "cells": [make_nb_cell(ct, src) for ct, src in CELLS],
}

out_path = r"C:\Users\Joseph\Desktop\projects\ANRF-AISEHack-2.0\submission_R2\15\anrf-v15.ipynb"
