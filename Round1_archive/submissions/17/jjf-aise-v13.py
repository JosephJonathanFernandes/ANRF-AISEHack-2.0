...
# -----------------------------------------------------------------------------
# 3. CHEMBERTA FINE-TUNING (ENSEMBLE SEEDS + EMBEDDING EXTRACTION)
# -----------------------------------------------------------------------------
print('\nPreparing ChemBERTa-2...')
model_name = "DeepChem/ChemBERTa-77M-MTR"
tokenizer = AutoTokenizer.from_pretrained(model_name)

class SmilesDataset(Dataset):
    def __init__(self, smiles, targets=None, max_len=128):
        self.smiles = smiles
...
CHEMBERTA_SEEDS = [42, 7, 123]

def train_chemberta_ensemble(df_train, df_test, y_true, target_name, seed, n_splits=5, epochs=15, batch_size=16, lr=2e-5):
    torch.manual_seed(seed)
    np.random.seed(seed)
    ...
    # Hidden dimension for ChemBERTa-77M is 384
    hidden_dim = 384
    oof_embs = np.zeros((len(df_train), hidden_dim))
    test_embs = np.zeros((len(df_test), hidden_dim))
    
    test_dataset = SmilesDataset(df_test['smiles'].tolist())
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    
    for fold, (train_idx, val_idx) in enumerate(kf.split(df_train)):
        print(f"    ChemBERTa fold {fold+1}/{n_splits} | {target_name} | Seed {seed}", flush=True)
        ...
        train_dataset = SmilesDataset(train_smiles, train_y)
        val_dataset   = SmilesDataset(val_smiles, val_y)
    ...
    print(f"  OOF ChemBERTa R² ({target_name} Seed {seed}): {r2_score(y_true, oof_preds):.4f}")
...
for seed in CHEMBERTA_SEEDS:
    oof_c, test_c, oof_e, test_e = train_chemberta_ensemble(train_tg, test_tg, y_tg, "Tg", seed=seed)
...
oof_e_tg  = np.mean(chemberta_oof_tg_embs_all, axis=0)
test_e_tg = np.mean(chemberta_test_tg_embs_all, axis=0)

chemberta_oof_egc_all, chemberta_test_egc_all = [], []
chemberta_oof_egc_embs_all, chemberta_test_egc_embs_all = [], []
for seed in CHEMBERTA_SEEDS:
    oof_c, test_c, oof_e, test_e = train_chemberta_ensemble(train_egc, test_egc, y_egc, "Egc", seed=seed)
...
oof_e_egc  = np.mean(chemberta_oof_egc_embs_all, axis=0)
test_e_egc = np.mean(chemberta_test_egc_embs_all, axis=0)
...
# -----------------------------------------------------------------------------
# 5. TABULAR MODELS (LGBM + XGB) with ChemBERTa Embeddings appended
# -----------------------------------------------------------------------------
...
X_tg_comb = np.hstack([X_tg, oof_e_tg])
X_tg_test_comb = np.hstack([X_tg_test, test_e_tg])
...
X_egc_comb = np.hstack([X_egc, oof_e_egc])
X_egc_test_comb = np.hstack([X_egc_test, test_e_egc])
...
w_tg  = find_best_weights((oof_l_tg, oof_x_tg, oof_g_tg, oof_c_tg),   y_tg)
w_egc = find_best_weights((oof_l_egc, oof_x_egc, oof_g_egc, oof_c_egc), y_egc)

print('\n' + '=' * 60)
print('  v13 ENSEMBLE')
print('=' * 60)
print(f'Optimal Tg  weights — LGBM: {w_tg[0]:.3f}  XGB: {w_tg[1]:.3f}  Large GNN: {w_tg[2]:.3f}  ChemBERTa-FT: {w_tg[3]:.3f}')
print(f'Optimal Egc weights — LGBM: {w_egc[0]:.3f}  XGB: {w_egc[1]:.3f}  Large GNN: {w_egc[2]:.3f}  ChemBERTa-FT: {w_egc[3]:.3f}')

oof_tg_opt  = w_tg[0]*oof_l_tg   + w_tg[1]*oof_x_tg   + w_tg[2]*oof_g_tg   + w_tg[3]*oof_c_tg
oof_egc_opt = w_egc[0]*oof_l_egc  + w_egc[1]*oof_x_egc  + w_egc[2]*oof_g_egc  + w_egc[3]*oof_c_egc
...
pred_tg  = w_tg[0]*test_l_tg   + w_tg[1]*test_x_tg   + w_tg[2]*test_g_tg   + w_tg[3]*test_c_tg
pred_egc = w_egc[0]*test_l_egc  + w_egc[1]*test_x_egc  + w_egc[2]*test_g_egc  + w_egc[3]*test_c_egc
...
submission.to_csv('outputs/submission_v13.csv', index=False)
print('\noutputs/submission_v13.csv saved.')
...
plt.suptitle(f'v13 OOF Predicted vs Actual  |  Mean R² = {(r2_tg+r2_egc)/2:.4f}', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('outputs/oof_scatter_v13.png', dpi=120, bbox_inches='tight')
print('Plot saved as outputs/oof_scatter_v13.png.')
