...
def train_chemberta_ensemble(df_train, df_test, y_true, target_name, n_splits=5, epochs=6, batch_size=16, lr=2e-5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(df_train)):
        print(f"    ChemBERTa fold {fold+1}/{n_splits} | {target_name}", flush=True)
    ...
    print(f"  OOF ChemBERTa R² ({target_name}): {r2_score(y_true, oof_preds):.4f}")
    return oof_preds, test_preds

print('=' * 60)
print('  ChemBERTa Fine-tuning')
print('=' * 60)
oof_c_tg,  test_c_tg  = train_chemberta_ensemble(train_tg,  test_tg,  y_tg,  "Tg")
oof_c_egc, test_c_egc = train_chemberta_ensemble(train_egc, test_egc, y_egc, "Egc")
...
# v10 update: Scaled up GNN parameters
...
submission.to_csv('outputs/submission_v10.csv', index=False)
print('\noutputs/submission_v10.csv saved.')
...
plt.suptitle(f'v10 OOF Predicted vs Actual  |  Mean R² = {(r2_tg+r2_egc)/2:.4f}', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('outputs/oof_scatter_v10.png', dpi=120, bbox_inches='tight')
print('Plot saved as outputs/oof_scatter_v10.png.')
