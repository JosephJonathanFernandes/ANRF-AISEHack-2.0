...
# -----------------------------------------------------------------------------
# 3. CHEMBERTA FINE-TUNING (ENSEMBLE SEEDS)
# -----------------------------------------------------------------------------
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(df_train)):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    attention_mask = batch['attention_mask'].to(DEVICE)
                    outputs = model(input_ids, attention_mask=attention_mask)
                    val_preds_fold.extend(outputs.logits.squeeze().cpu().numpy().tolist())
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                attention_mask = batch['attention_mask'].to(DEVICE)
                outputs = model(input_ids, attention_mask=attention_mask)
                test_preds_fold.extend(outputs.logits.squeeze().cpu().numpy().tolist())
...
for seed in CHEMBERTA_SEEDS:
    oof_c, test_c = train_chemberta_ensemble(train_tg, test_tg, y_tg, "Tg", seed=seed)
...
for seed in CHEMBERTA_SEEDS:
    oof_c, test_c = train_chemberta_ensemble(train_egc, test_egc, y_egc, "Egc", seed=seed)
...
# v11 update: Scaled up GNN parameters
...
print('\n' + '=' * 60)
print('  Tabular (LGBM+XGB)')
print('=' * 60)

tg_oof_parts_all, tg_test_parts_all = [], []
for seed in SEEDS:
    oof_parts, test_parts = train_tabular_ensemble(X_tg, y_tg, X_tg_test, 'tg', seed=seed)
...
for seed in SEEDS:
    oof_parts, test_parts = train_tabular_ensemble(X_egc, y_egc, X_egc_test, 'egc', seed=seed)
...
submission.to_csv('outputs/submission_v12.csv', index=False)
print('\noutputs/submission_v12.csv saved.')
...
plt.suptitle(f'v12 OOF Predicted vs Actual  |  Mean R² = {(r2_tg+r2_egc)/2:.4f}', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('outputs/oof_scatter_v12.png', dpi=120, bbox_inches='tight')
print('Plot saved as outputs/oof_scatter_v12.png.')
